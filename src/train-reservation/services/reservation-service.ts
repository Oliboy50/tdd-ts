...
export class ReservationService implements IReservationService {
  constructor(
    @Inject('IBookingReferenceService')
    private readonly bookingReferenceService: IBookingReferenceService,
    @Inject('ITrainDataService')
    private readonly trainDataService: ITrainDataService,
  ) {}
  ...
  private static get_bookable_seats(number_seats, train: ITrain): ISeat[] {
    ...
    const [overallReservedSeats, overallEmptySeats] = partition(
      seat => seat.is_reserved,
      overallSeats,
    );

    const trainHasEnoughEmptySeats = number_seats <= overallEmptySeats.length;
    const trainWouldHaveMoreThan70PercentReservedSeats =
      overallReservedSeats.length + number_seats >
      Math.ceil(overallSeats.length * 0.7);

    if (
      !trainHasEnoughEmptySeats ||
      trainWouldHaveMoreThan70PercentReservedSeats
    ) {
  ...
  async book_seats(
    ...
    const bookable_seats = ReservationService.get_bookable_seats(
      number_seats,
      train_topology,
    );
