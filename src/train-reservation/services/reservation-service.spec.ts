...
describe('ReservationService', () => {
  ...
  beforeEach(async () => {
    ...
    provider = new ReservationService(
      bookingReferenceService,
      trainDataService,
    );
  });
  ...
  describe('book_seats', () => {
    describe('when the train is empty', () => {
      ...
      it('fails when we want to book more seats than available', async () => {
        ...
        const trainId = 'fooBar';
        await expect(provider.book_seats(trainId, 10)).rejects.toEqual(
          new Error('Train capacity too low'),
        );
      });
    });
    ...
    describe('when the train would reached > 70% capacity', () => {
      it('should throw an exception', async () => {
        ...
        const trainId = 'fooBar';
        await expect(provider.book_seats(trainId, 3)).rejects.toEqual(
          new Error('Train capacity too low'),
        );
