...
export class TrainDataService implements ITrainDataService {
  ...
  private static service_url: string = process.env["TDS_URL"] || "http://localhost:8081";
  ...
  get_train_data(train_id: string): Promise<ITrain> {
    ...
    return TrainDataService.client.get(`data_for_train/${train_id}`)
      ...
      .then(apiReturn => {
        ...
        const coaches: Coach[] =
          toPairs(groupedSeats).map(([coachName, seats]) =>
            new Coach(
              coachName,
              seats.map(s => new Seat(Number(s.seat_number), coachName, s.booking_reference))
            )
          );
